  ...
  for (var i = 0; i < blacklist2.length; i++) {
    if (currentURL.indexOf(blacklist2[i]) !== -1) {
      (function() {
        ...
        var toastMessage = document.createTextNode('Sorry! Zenly isn\'t supported for i-Ready, but it will be soon! 🥳');
  ...
  for (var i = 0; i < blacklist.length; i++) {
    if (currentURL.indexOf(blacklist[i]) !== -1) {
      (function() {
        ...
        toastContainer.style.transform = 'translateX(350px)';
        toastContainer.style.zIndex = '99999999999999';
        var toastMessage = document.createTextNode('Whoops! Zenly can\'t connect to this domain because it\'s flagged as unsupported. Please use Zenly on a different website!');
        toastContainer.appendChild(toastMessage);
        document.body.appendChild(toastContainer);
        setTimeout(function() {
          toastContainer.style.transform = 'translateX(0)';
